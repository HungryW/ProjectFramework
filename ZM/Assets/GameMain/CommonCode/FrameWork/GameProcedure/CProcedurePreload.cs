...
namespace GameFrameworkPackage
{
    public class CProcedurePreload : CProcedureBase
    {
        ...
        private Dictionary<string, string[]> m_loadedHotfixConfig = new Dictionary<string, string[]>(); //热更新配置 比如数据表名列表
        private Dictionary<string, byte[]> m_loadedHotifx = new Dictionary<string, byte[]>();    //热更新程序
        ...
        private void _LoadHotfixRes()
        {
            _LoadHotFixConfig(CAssestPathUtility.HotfixDataTableListName);
            _LoadHotFixConfig(CAssestPathUtility.HotfixUISpriteListName);
            _LoadHotFixConfig(CAssestPathUtility.HotfixLocalizationListName);
            _LoadHotfixScripts();
        }
        ...
        private void _StartLoadAfterRes()
        {
            ...
            //CTextToolSensitiveCheck.Instance.InitData();
            //_LoadUIs();
            //_LoadSpineActors();
        }


        private void _StartHotFixAssembly()
        {
            byte[] dllBytes, pdbBytes;
            m_loadedHotifx.TryGetValue(CAssestPathUtility.HotfixDllName, out dllBytes);
            m_loadedHotifx.TryGetValue(CAssestPathUtility.HotfixPdbName, out pdbBytes);
        }
        ...
        private void _LoadDataTables()
        {
            ...
            string[] arrName;
            if (m_loadedHotfixConfig.TryGetValue(CAssestPathUtility.HotfixDataTableListName, out arrName))
            {
                for (int i = 0; i < arrName.Length; i++)
                {
                    _LoadDataTable(arrName[i]);
                }
            }
            else
            {
                Log.Error("_LoadDataTables Fail");
            }
        }
        ...
        {
            string[] arrName;
            if (m_loadedHotfixConfig.TryGetValue(CAssestPathUtility.HotfixLocalizationListName, out arrName))
            {
                for (int i = 0; i < arrName.Length; i++)
                {
                    _LoadDictionary(arrName[i]);
                }
            }
            else
            {
                Log.Error("_LoadDictionarys Fail");
            }
        ...
        private void _LoadUISprites()
        {
            CUISpriteMgr.Clean();
            string[] arrName;
            if (m_loadedHotfixConfig.TryGetValue(CAssestPathUtility.HotfixUISpriteListName, out arrName))
            {
                for (int i = 0; i < arrName.Length; i++)
                {
                    _LoadUISprite(arrName[i]);
                }
            }
            else
            {
                Log.Error("_LoadUISprites Fail");
            }


        }

        private void _LoadHotFixConfig(string a_szName)
        {
            m_LoadedFlag.Add(Utility.Text.Format("HotFixConfig.{0}", a_szName), false);
            CGameEntryMgr.Resource.LoadAsset(CAssestPathUtility.GetHotfixAsset(a_szName),
                new LoadAssetCallbacks(
                    (assetName, asset, duration, userData) =>
                    {

                        m_loadedHotfixConfig.Add(a_szName, CPackageUtility.Text.SplitToLines(((TextAsset)asset).text));
                        _SetLoaded(Utility.Text.Format("HotFixConfig.{0}", a_szName));
                        Log.Info("Load HotFixConfig '{0}' OK.", a_szName);
                    },
        ...
        private void _LoadHotfixScripts()
        {
            //加载热更资源
            _LoadHotFix(CAssestPathUtility.HotfixDllName);
            _LoadHotFix(CAssestPathUtility.HotfixPdbName);
        }

        private void _LoadHotFix(string a_szfileName)
        {
            string name = Utility.Text.Format("HotFix.{0}", a_szfileName);
            m_LoadedFlag.Add(name, false);
            CGameEntryMgr.Resource.LoadAsset(CAssestPathUtility.GetHotfixAsset(a_szfileName),
               new LoadAssetCallbacks(
                   //加载成功的回调
                   (assetName, asset, duration, userData) =>
                   {
                       m_LoadedFlag[name] = true;
                       m_loadedHotifx.Add(a_szfileName, ((TextAsset)asset).bytes);
                       Log.Info("Load hotfix '{0}' OK.", a_szfileName);
                   },
                   //加载失败的回调
                   (assetName, status, errorMessage, userData) =>
                   {
                       Log.Error("Can not load hotfix '{0}' from '{1}' with error message '{2}'.", a_szfileName, assetName, errorMessage);
                   }
           ));
        }
