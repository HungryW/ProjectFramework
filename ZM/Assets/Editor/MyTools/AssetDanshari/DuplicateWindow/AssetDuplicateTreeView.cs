        ...
        protected override void CellGUI(Rect cellRect, AssetTreeViewItem<AssetTreeModel.AssetInfo> item, int column, ref RowGUIArgs args)
        {
            ...
            switch (column)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    TextureImporter target = AssetDatabase.LoadAssetAtPath<TextureImporter>(info.fileRelativePath);
                    if (target == null)
                    {
                        break;
                    }
                    long runtimeMemorySize = Profiler.GetRuntimeMemorySizeLong(target);

                    DefaultGUI.Label(cellRect, AssetDanshariUtility.GetFileSizeStr(runtimeMemorySize), args.selected, args.focused);
