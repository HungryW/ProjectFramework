...
namespace GameFrameworkPackage
{
    ...
    [ExecuteInEditMode]
    //添加选项到菜单中
    [AddComponentMenu("Unlit/UIBlurShader")]
    public class CUIBlurEffect : MonoBehaviour
    {
        //-------------------变量声明部分-------------------
        #region Variables

        //指定Shader名称
        private string ShaderName = "Unlit/UIBlurShader";
        ...
        [Range(0, 30), Tooltip("1")]
        public int MainRange1 = 10;

        [Range(0, 30), Tooltip("2")]
        public int MainRange2 = 10;

        private int MainRange = 1;
        ...
        {
            ////依次赋值
            //ChangeValue = Mathf.Abs(DownSampleNum);
            //ChangeValue2 = Mathf.Abs(BlurSpreadSize);
            //ChangeValue3 = Mathf.Abs(BlurIterations);
            MainRange = MainRange1 + MainRange2;
            ChangeValue = Mathf.Abs(DownSampleNum);
            ChangeValue2 = Mathf.Abs(MainRange/10f);
            ChangeValue3 = Mathf.Abs(MainRange/10);
        ...
        void OnRenderImage(RenderTexture sourceTexture, RenderTexture destTexture)
        {
            ...
            if (CurShader != null)
            {
                ...
                float widthMod = 1.0f / (1.0f * (1 << Mathf.Abs(DownSampleNum)));
                //Shader的降采样参数赋值
                material.SetFloat("_DownSampleValue", MainRange/10f * widthMod);
                //设置渲染模式：双线性
                sourceTexture.filterMode = FilterMode.Bilinear;
                //通过右移，准备长、宽参数值
                int renderWidth = sourceTexture.width >> Mathf.Abs(DownSampleNum);
                int renderHeight = sourceTexture.height >> Mathf.Abs(DownSampleNum);
                ...
                for (int i = 0; i < Mathf.Abs(MainRange/10); i++)
                {
                    //【2.1】Shader参数赋值
                    //迭代偏移量参数
                    float iterationOffs = (i * 1.0f);
                    //Shader的降采样参数赋值
                    material.SetFloat("_DownSampleValue", MainRange/10f * widthMod + iterationOffs);
